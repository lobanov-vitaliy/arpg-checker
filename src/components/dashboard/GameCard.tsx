...
interface GameCardProps {
  game: GameConfig;
  prefetchedSeasons?: SeasonData[];
  prefetchedLikes?: number;
  prefetchedSteam?: SteamData | null;
}

export async function GameCard({ game, prefetchedSeasons, prefetchedLikes, prefetchedSteam }: GameCardProps) {
  const t = await getTranslations("dashboard");
  const tPop = await getTranslations("popularity");

  const [locale, allSeasons, likesCount] = await Promise.all([
    getLocale(),
    prefetchedSeasons !== undefined ? Promise.resolve(prefetchedSeasons) : getSeasonsForGame(game.id),
    prefetchedLikes !== undefined ? Promise.resolve(prefetchedLikes) : getLikesCount(game.id),
  ]);
  const isFirstSeason =
    allSeasons.length === 1 &&
    allSeasons[0].seasonNumber === 1 &&
    (allSeasons[0].status === "active" || allSeasons[0].status === "upcoming");
  const steamData = prefetchedSteam !== undefined
    ? prefetchedSteam
    : game.steamAppId
